		...
		)
{
	...
	uint32_t cpunum, mem_region_num;
	uint64_t linear_id;
	 ...
	 * Mp information hob.
	 */
	ret = plat_get_all_processor_id(&processor_id, &cpunum);
	if (ret < 0) {
		ERROR("Failed to get processor id. ret:%d\n", ret);
		return NULL;
	}

	ret = create_guid_hob(hob_table, &mpinfo_guid,
			(sizeof(struct efi_mp_information_hob_data) +
			(sizeof(struct efi_processor_information) * cpunum)),
			(void **) &mp_data);
	if (ret < 0) {
		ERROR("Failed to create mp informatio hob. ret:%d\n", ret);
		return NULL;
	}

	mp_data->number_of_processors = cpunum;
	mp_data->number_of_enabled_processors = cpunum;
	for (node = 0; node < cpunum; node++) {
		processor_info = &mp_data->processor_info[node];
		memset(processor_info, 0x00, sizeof(struct efi_processor_information));
		linear_id = plat_core_pos_by_mpidr(processor_id[node]);

		processor_info->processor_id = processor_id[node];
		if (processor_id[node] & MPIDR_MT_MASK) {
			processor_info->location.package = MPIDR_AFFLVL2_VAL(processor_id[node]);
			processor_info->location.core = MPIDR_AFFLVL1_VAL(processor_id[node]);
			processor_info->location.thread = MPIDR_AFFLVL0_VAL(processor_id[node]);
		} else {
			processor_info->location.package = MPIDR_AFFLVL1_VAL(processor_id[node]);
			processor_info->location.core = MPIDR_AFFLVL0_VAL(processor_id[node]);
