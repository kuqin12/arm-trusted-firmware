...
MM_COMM_BUFFER_GUID = (
    ...
    0x0131,
    0x4AEE,
    (0xA7, 0x50, 0xCC, 0x38, 0x4A, 0xAC, 0xE8, 0xC6),
)
...
def get_memory_region_base_address(node, load_address):
...
class EFI_GUID:
    ...
    {hex(self.time_hi_and_version)}, {self.clock_seq_and_node}"
...
class Hob_Guid:
    ...
    def __init__(self, name: EFI_GUID, data_format_str, data):
        ...
        self.header = Hob_Generic_Header(EFI_HOB_TYPE_GUID_EXTENSION, hob_length)
        self.name = name
        self.data = data
...
class Handoff_Info_Table:
    ...
    def __init__(self, memory_base, memory_size, free_memory_base, free_memory_size):
        ...
        self.free_memory_bottom = free_memory_base
        self.hob_end = None
...
def generate_shared_buf_guid(base_addr, page_count):
    shared_buf_desc = ("2Q", (base_addr, page_count))
    return Hob_Guid(EFI_GUID(*MM_COMM_BUFFER_GUID), *shared_buf_desc)
...
def generate_hob_from_fdt_node(sp_fdt, hob_offset, hob_size=None):
    ...
    entrypoint_offset = get_uint32_property_value(sp_fdt, "entrypoint-offset")
    ...
    fv_hob = Firmware_Volume_Hob(load_address, entrypoint_offset, img_size)
    hob_list_base = load_address + hob_offset
    # TODO assuming one page allocated for HOB List
    max_table_size = hob_size if hob_size is not None else (1 << PAGE_SIZE_SHIFT)
    phit = Handoff_Info_Table(
    ...
    hob_list.get_phit().set_hob_end_addr(
        hob_list_base
        + struct_calcsize_with_endianness(phit.header.format_str + hob_list.format_str)
    )
    hob_list.get_phit().set_free_memory_bottom_addr(
        hob_list.get_phit().hob_end
        + struct_calcsize_with_endianness(end_hob.header.format_str)
    )
